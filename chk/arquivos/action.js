...
function startchk(url) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.d.Result.includes("problems")) {
                remover_linha("#lista");
                document.getElementById("testado").innerHTML = (eval(document.getElementById("testado").innerHTML) + 1);
                $("#reprovadas").append("<span class='label label-danger'>#Reprovada ❌ </span> " + url + " #GiovaneVoltouEMereceUmaChupada <br/>");
                $("#reprovada_conta").text(countlixo);
                $("#reprovada_conta_2").text(countlixo);
            }
            else if (data.d.Result.includes("transactionId")) {
                remover_linha("#lista");
                document.getElementById("testado").innerHTML = (eval(document.getElementById("testado").innerHTML) + 1);
                $("#aprovadas").append('<span class="label label-success">#Aprovada ✅</span> ' + url + ' #GiovaneVoltouEMereceUmaChupada <br/>');
